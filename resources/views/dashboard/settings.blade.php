...
<div class="bg-gradient-to-br from-white to-indigo-50 rounded-xl shadow-md p-6 mb-8 border border-indigo-100">
    <h2 class="text-xl font-bold mb-6">Profile Photo</h2>
        ...
        <div class="flex-1">
            <form action="{{ route('dashboard.profile.photo.update') }}" method="POST" enctype="multipart/form-data" id="photoForm">
                ...
                <div class="flex space-x-3">
                    <button type="button" onclick="document.getElementById('profile_photo').click()" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 shadow-md hover:shadow-lg transition-all">
                        <i class="fas fa-{{ auth()->user()->profile_photo ? 'edit' : 'upload' }} mr-2"></i>{{ auth()->user()->profile_photo ? 'Change Photo' : 'Upload Photo' }}
                    </button>
                    @if(auth()->user()->profile_photo)
                        <button type="button" onclick="confirmDelete()" class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 shadow-md hover:shadow-lg transition-all">
...
<div class="grid md:grid-cols-2 gap-8">
    ...
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <form action="{{ route('dashboard.password.update') }}" method="POST" class="space-y-4" id="passwordForm">
            @csrf
            <div>
            ...
            <div>
                ...
                <div id="new-password-strength" class="mt-2 text-sm"></div>
            </div>
            ...
            <button type="submit" id="passwordSubmitBtn" disabled class="bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700 disabled:bg-gray-400 disabled:cursor-not-allowed">
                Change Password
            </button>
        </form>
    </div>
