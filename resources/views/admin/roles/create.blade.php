    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="text" name="guard_name" id="guard_name" class="form-control @error('guard_name') is-invalid @enderror" value="{{ old('guard_name', 'web') }}" placeholder="web">
                @error('guard_name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                                ...
                                @foreach($permissions as $permission)
                                    <div class="col-md-4 col-sm-6">
                                        <div class="form-check mb-2">
                                            <input type="checkbox" class="form-check-input permission-checkbox" name="permissions[]" value="{{ $permission->id }}" id="permission-{{ $permission->id }}" data-group="{{ Str::slug($group) }}" {{ in_array($permission->id, old('permissions', [])) ? 'checked' : '' }}>
