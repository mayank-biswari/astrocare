...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid md:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <div class="border-b pb-4 mb-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h3 class="font-bold">{{ ucfirst($kundliData['type']) }} Kundli</h3>
                        <p class="text-gray-600">For: {{ $kundliData['name'] }}</p>
                        <p class="text-indigo-600 font-bold">₹{{ number_format($kundliData['amount'], 2) }}</p>
                    </div>
                </div>
            </div>
            <div class="text-right">
                <p class="text-2xl font-bold">Total: ₹{{ number_format($kundliData['amount'], 2) }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <form action="{{ route('kundli.order.place') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <select name="country_code" id="country_code" required
                                class="w-32 px-2 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm">
                            @foreach($countryCodes as $country)
                                <option value="{{ $country->dial_code }}"
                                        data-digits="{{ $country->phone_digits }}"
                                        {{ old('country_code', '+91') == $country->dial_code ? 'selected' : '' }}>
                                    {{ $country->flag }} {{ $country->dial_code }}
                                </option>
                            @endforeach
                        </select>
                        <input type="tel" name="phone" id="phone_input" value="{{ old('phone', auth()->user()->phone ?? '') }}" required
                               placeholder="Mobile number"
                               class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <p class="text-xs text-gray-500 mt-1" id="phone_hint"></p>
                    @error('phone')
                ...
                <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
                    <p class="text-sm text-blue-700">
                        ...
                        Your kundli will be generated and available in your dashboard after payment confirmation.
                    </p>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePhoneHint() {
        ...
        const digits = selected.getAttribute('data-digits');
        if (digits) {
            phoneInput.setAttribute('maxlength', digits);
            phoneHint.textContent = 'Enter ' + digits + ' digit mobile number';
        }
    }
